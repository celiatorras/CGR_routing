...
function mv_unibo_cgr_to_ion() {
	...
	cp -pf "$AUX_BPV6/bpv6_Makefile"  "$ION_BPV6/"
	...
	cp -pf "$AUX_BPV6/Makefile.noBP"  "$ION/"
...
#	rm -rf "$ION_BPV6/cgr/Unibo-CGR/ion_bpv6/extensions"
	...
	cp -pf "$AUX_BPV7/bpv7_Makefile"  "$ION_BPV7/"
	cp -pf "$AUX_BPV7/cgr.h"          "$ION_BPV7/library/"
	cp -pf "$AUX_BPV7/cgrfetch.c"     "$ION_BPV7/utils/"
	cp -pf "$AUX_BPV7/configure.ac"   "$ION/"
	cp -pf "$AUX_BPV7/ipnfw.c"        "$ION_BPV7/ipn/"
	cp -pf "$AUX_BPV7/libcgr.c"       "$ION_BPV7/cgr/"
	cp -pf "$AUX_BPV7/Makefile.am"    "$ION/"
	cp -pf "$AUX_BPV7/Makefile.noBP"  "$ION/"
